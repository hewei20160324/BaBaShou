...
public class TrackItemControl : DirectorEditor.DirectorBehaviourControl
{
    ...
    internal void BoxSelect(Rect selectionBox)
    {
        ...
        else if (Selection.Contains(this.wrapper.Behaviour.gameObject))
        {
            ...
            ArrayUtility.Remove<GameObject>(ref array, this.wrapper.Behaviour.gameObject);
...
//     }
    ...
    protected virtual void DrawRenameLabel(string name, Rect labelPosition, GUIStyle labelStyle = null)
    {
        if (this.isRenaming)
        {
            ...
            if ((!EditorGUIUtility.editingTextField || (this.renameControlID != GUIUtility.keyboardControl))
                || ((Event.current.keyCode == KeyCode.Return) || ((Event.current.type == EventType.MouseDown) && !labelPosition.Contains(Event.current.mousePosition))))
            {
