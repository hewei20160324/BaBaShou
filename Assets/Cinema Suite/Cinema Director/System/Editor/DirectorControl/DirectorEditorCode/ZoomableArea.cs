...
public class ZoomableArea
{
    ...
    public void BeginViewGUI(bool handleUserInteraction)
    {
        ...
        {
            int num = GUIUtility.GetControlID(zoomableAreaHash, FocusType.Native, drawArea);
    ...
    internal Rect rect
    {
        get { return new Rect(this.drawRect.x, this.drawRect.y, this.drawRect.width, this.drawRect.height + (!this.m_HSlider ? 0f : this.styles.visualSliderWidth)); }
        set
        {
    ...
    {
        get { return new Rect(-this.m_Translation.x / this.m_Scale.x, -(this.m_Translation.y - this.drawRect.height) / this.m_Scale.y, this.drawRect.width / this.m_Scale.x, this.drawRect.height / -this.m_Scale.y); }
        set
        {
