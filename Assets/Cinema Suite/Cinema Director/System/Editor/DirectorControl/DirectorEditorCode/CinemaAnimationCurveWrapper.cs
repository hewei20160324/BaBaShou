...
public class CinemaAnimationCurveWrapper
{
    ...
    public void AddKey(float time, float value, int tangentMode)
    {
        ...
        keyframe3.tangentMode = (tangentMode);
        ...
        {
            if (this.IsLeftLinear(index))
            {
                this.SetKeyLeftLinear(index);
            }
            if (this.IsRightLinear(index))
            {
                this.SetKeyRightLinear(index);
            }
            if (this.IsLeftConstant(index))
            {
                this.SetKeyLeftConstant(index);
            }
            if (this.IsRightConstant(index))
            {
                this.SetKeyRightConstant(index);
            }
        }
        if (index > 0)
        {
            this.curve.MoveKey(index - 1, keyframe);
    ...
    public float Evaluate(float time) { return this.curve.Evaluate(time); }

    internal void FlattenKey(int index)
    {
        Keyframe keyframe = new Keyframe(this.curve[index].time, this.curve[index].value, 0f, 0f);
    ...
    internal void Flip()
    {
        if (this.curve.length >= 2)
        {
            ...
            float num = this.curve[0].time;
            float num2 = this.curve[this.curve.length - 1].time;
    ...
    { return (this.curve[index].tangentMode == 0); }
    ...
    public int KeyframeCount { get { return this.curve.length; } }
}
