...
public class DirectorControl : TimeArea
{
    ...
    //private List<DirectorEditor.SidebarControl> sidebarControls = new List<DirectorEditor.SidebarControl>();
    ...
    {
        List<DirectorEditor.SidebarControl> newSidebarControls = new List<DirectorEditor.SidebarControl>();
        List<DirectorEditor.SidebarControl> removedSidebarControls = new List<DirectorEditor.SidebarControl>();
        ...
        foreach (DirectorEditor.SidebarControl local1 in newSidebarControls)
        {
            local1.DeleteRequest += new DirectorEditor.DirectorBehaviourControlHandler(this.control_DeleteRequest);
            local1.DuplicateRequest += new DirectorEditor.SidebarControlHandler(this.sidebarControl_Duplicate);
            local1.SelectRequest += new DirectorEditor.SidebarControlHandler(this.sidebarControl_SelectRequest);
        }
        foreach (DirectorEditor.SidebarControl local2 in removedSidebarControls)
        {
            local2.DeleteRequest -= new DirectorEditor.DirectorBehaviourControlHandler(this.control_DeleteRequest);
            local2.DuplicateRequest -= new DirectorEditor.SidebarControlHandler(this.sidebarControl_Duplicate);
            local2.SelectRequest -= new DirectorEditor.SidebarControlHandler(this.sidebarControl_SelectRequest);
        }
        foreach (TrackItemControl control in newTimelineControls)
        {
            this.timelineControls.Add(control);
            control.DeleteRequest += new DirectorEditor.DirectorBehaviourControlHandler(this.control_DeleteRequest);
        ...
        foreach (TrackItemControl control2 in removedTimelineControls)
        {
            ...
            control2.DeleteRequest -= new DirectorEditor.DirectorBehaviourControlHandler(this.control_DeleteRequest);
    ...
    private void bindTrackGroupControls(CutsceneWrapper cutscene, List<DirectorEditor.SidebarControl> newSidebarControls, List<DirectorEditor.SidebarControl> removedSidebarControls, List<TrackItemControl> newTimelineControls, List<TrackItemControl> removedTimelineControls)
    {
    ...
    private void sidebarControl_Duplicate(object sender, DirectorEditor.SidebarControlEventArgs e)
    {
        foreach (TrackGroupWrapper wrapper in this.trackGroupBinding.Keys)
        {
            TrackGroupControl control = this.trackGroupBinding[wrapper];
    ...
    // 选中逻辑;
    private void sidebarControl_SelectRequest(object sender, DirectorEditor.SidebarControlEventArgs e)
    {
        ...
        if (behaviour != null)
        {
            if (Event.current.control)
            {
                ...
                else
                {
                    ...
                    ArrayUtility.Add<GameObject>(ref array, behaviour.gameObject);
            ...
            else if (Event.current.shift)
            {
                ...
                DirectorEditor.SidebarControl sidebarControl = e.SidebarControl;
                DirectorEditor.SidebarControl control2 = e.SidebarControl;
                foreach (DirectorEditor.SidebarControl control4 in list)
                {
                ...
                foreach (DirectorEditor.SidebarControl control5 in list)
                {
    ...
    private void updateControlLayout(Rect controlArea)
    {
        ...
        // 定义一块触摸控件，不接受按键，只接受鼠标;
        // 实现左右两块编辑器比例分配逻辑;
        int num = GUIUtility.GetControlID("SidebarResize".GetHashCode(), FocusType.Passive, this.sidebarControlArea);
    ...
    private void updateDragAndDrop()
    {
        ...
        if (event2.type == EventType.DragExited)
        {
    ...
    private void updateDurationBar()
    {
        ...
        int num2 = GUIUtility.GetControlID("DurationBar".GetHashCode(), FocusType.Passive, rect);
    ...
    private void updateTimelineHeader(Rect headerArea, Rect timeRulerArea)
    {
        ...
        if (this.cutscene != null)
        {
            int num = GUIUtility.GetControlID("TimeRuler".GetHashCode(), FocusType.Passive, timeRulerArea);
            switch (Event.current.GetTypeForControl(num))
            {
                case EventType.MouseDown:
    ...
    {
        int num = GUIUtility.GetControlID("DirectorBody".GetHashCode(), FocusType.Passive, this.trackBodyBackgroundNoVerticalScrollbar);
        switch (Event.current.GetTypeForControl(num))
        {
            case EventType.MouseDown:
    ...
    {
        get { return this.directorState.DefaultTangentMode; }
        set
        {
            this.directorState.DefaultTangentMode = value;
        }
    }

    public bool InPreviewMode
    {
        get { return this.directorState.IsInPreviewMode; }
        set
        {
            if (this.cutscene != null)
            {
    ...
    {
        get { return this.directorState.IsSnapEnabled; }
        set
        {
    ...
    {
        get { return this.directorState.ResizeOption; }
        set
        {
