...
public abstract class CinemaCurveClipItemControl : ActionItemControl
{
    ...
    private void addKeyframes(object userData)
    {
        ...
        if (context != null)
        {
            ...
            for (int i = 0; i < memberCurves.Length; i++)
            {
                ...
                {
                    wrapper.AddKey(context.time, wrapper.Evaluate(context.time), context.state.DefaultTangentMode);
                }
            }
    ...
    private void addKeyToCurve(object userData)
    {
        ...
        if (context != null)
        {
            context.curveWrapper.AddKey(context.time, context.curveWrapper.Evaluate(context.time), context.state.DefaultTangentMode);
            if (this.CurvesChanged != null)
            {
...
//         view.y;
    ...
    private void handleItemInput(CinemaClipCurveWrapper clipCurveWrapper, DirectorControlState state, Rect trackPosition)
    {
        ...
        Rect position = new Rect(num, 0f, 5f, this.timelineItemPosition.height);
        ...
        switch (Event.current.GetTypeForControl(num3))
        {
            ...
            case EventType.MouseDrag:
                if (GUIUtility.hotControl == num3)
                {
                    ...
                    foreach (CinemaActionWrapper wrapper4 in base.Track.Items)
                    {
                        if ((wrapper4 != null) && (wrapper4.Behaviour != base.Wrapper.Behaviour))
                        {
                            float num10 = wrapper4.Firetime + wrapper4.Duration;
                    ...
                    if (state.ResizeOption == DirectorEditor.ResizeOption.Crop)
                    {
                        clipCurveWrapper.CropFiretime(time);
                    }
                    else if (state.ResizeOption == DirectorEditor.ResizeOption.Scale)
                    {
        ...
        switch (Event.current.GetTypeForControl(num5))
        {
            ...
            case EventType.MouseDrag:
                if (GUIUtility.hotControl == num5)
                {
                    ...
                    foreach (CinemaActionWrapper wrapper7 in base.Track.Items)
                    {
                        if ((wrapper7 != null) && (wrapper7.Behaviour != base.Wrapper.Behaviour))
                        {
                            float num13 = clipCurveWrapper.Firetime + clipCurveWrapper.Duration;
                            if (wrapper7.Firetime >= num13)
                            {
                                positiveInfinity = Mathf.Min(positiveInfinity, wrapper7.Firetime);
                            }
                        }
                    }
                    ...
                    CinemaClipCurveWrapper wrapper6 = base.Wrapper as CinemaClipCurveWrapper;
                    if (wrapper6 != null)
                    {
                        this.CurvesChanged(this, new CurveClipWrapperEventArgs(wrapper6));
                    }
                    this.haveCurvesChanged = true;
                }
                break;
    ...
    {
        float introduced21 = this.viewingSpace.height;
        float verticalRange = introduced21 - this.viewingSpace.y;
                        ...
                        switch (Event.current.GetTypeForControl(controlID))
                        {
                            ...
                            case EventType.MouseDrag:
                            {
                                if (((GUIUtility.hotControl == controlID) && (Event.current.button == 0)) && flag)
                                {
                                    ...
                                    float time = (Event.current.mousePosition.x - state.Translation.x) / state.Scale.x;
                                    float introduced22 = this.curveTrackSafeArea.y;
                                    float num7 = ((((introduced22 + this.curveTrackSafeArea.height) - Event.current.mousePosition.y) / this.curveTrackSafeArea.height) * verticalRange) + this.viewingSpace.y;
                                    time = state.SnappedTime(time);
                                    if (isBookEnd)
                                    {
                                        time = keyframe2.time;
                                    }
                                    float introduced23 = keyframe2.inTangent;
                                    Keyframe kf = new Keyframe(time, num7, introduced23, keyframe2.outTangent);
        ...
        foreach (CinemaMemberCurveWrapper wrapper in clipCurveWrapper.MemberCurves)
        {
            foreach (CinemaAnimationCurveWrapper wrapper2 in wrapper.AnimationCurves)
            {
                ...
                Label_0243:
                    if (GUIUtility.hotControl == controlID)
                    {
                        float introduced30 = this.curveTrackSafeArea.y;
                        float introduced31 = keyframe.time;
                        Vector2 vector2 = new Vector2((Event.current.mousePosition.x - state.Translation.x) / state.Scale.x, ((((introduced30 + this.curveTrackSafeArea.height) - Event.current.mousePosition.y) / this.curveTrackSafeArea.height) * verticalRange) + this.viewingSpace.y) - new Vector2(introduced31, keyframe.value);
                        ...
                        Keyframe kf = new Keyframe(introduced32, keyframe.value, num5, num6);
                ...
                Label_035D:
                    if (((i < (wrapper2.KeyframeCount - 1)) && !wrapper2.IsRightLinear(i)) && !wrapper2.IsRightConstant(i))
                    {
                        ...
                        num7 = GUIUtility.GetControlID("TangentOut".GetHashCode(), FocusType.Passive);
                ...
                Label_04D1:
                    if (GUIUtility.hotControl == num7)
                    {
                        float introduced33 = this.curveTrackSafeArea.y;
                        Vector2 vector4 = new Vector2((Event.current.mousePosition.x - state.Translation.x) / state.Scale.x, ((((introduced33 + this.curveTrackSafeArea.height) - Event.current.mousePosition.y) / this.curveTrackSafeArea.height) * verticalRange) + this.viewingSpace.y);
                        float introduced34 = keyframe.time;
                        Vector2 vector5 = new Vector2(introduced34, keyframe.value) - vector4;
                        ...
                        float introduced35 = keyframe.time;
                        Keyframe keyframe3 = new Keyframe(introduced35, keyframe.value, num9, num8);
                        keyframe3.tangentMode = (keyframe.tangentMode);
                        wrapper2.MoveKey(i, keyframe3);
                        CinemaClipCurveWrapper wrapper7 = base.Wrapper as CinemaClipCurveWrapper;
                        if (wrapper7 != null)
                        {
                            this.CurvesChanged(this, new CurveClipWrapperEventArgs(wrapper7));
                        }
                        this.haveCurvesChanged = true;
                    }
                }
                            ...
                            if (wrapper3.IsVisible)
                            {
                                for (int j = 0; j < wrapper3.KeyframeCount; j++)
                                {
                                    ...
                                    if (keyframe.time == num2)
                                    {
                                        ...
                                        float introduced24 = keyframe.value;
                                        float introduced25 = keyframe.inTangent;
                                        Keyframe kf = new Keyframe(num6, introduced24, introduced25, keyframe.outTangent);
                                        kf.tangentMode = (keyframe.tangentMode);
    ...
    internal void UpdateHeaderArea(DirectorControlState state, Rect controlHeaderArea)
    {
        ...
        if (wrapper != null)
        {
            ...
            foreach (CinemaMemberCurveWrapper wrapper2 in wrapper.MemberCurves)
            {
                ...
                string str2 = (userFriendlyName == string.Empty) ? wrapper2.Type : "wrapper2.Type.FriendlyName";
                ...
                int num4 = GUIUtility.GetControlID(base.Wrapper.Behaviour.GetInstanceID(), FocusType.Passive, rect5);
                if (((Event.current.GetTypeForControl(num4) == EventType.MouseDown) && rect5.Contains(Event.current.mousePosition)) && (Event.current.button == 0))
                {
                    GenericMenu menu1 = new GenericMenu();
                ...
                if (wrapper2.IsFoldedOut)
                {
                    foreach (CinemaAnimationCurveWrapper wrapper3 in wrapper2.AnimationCurves)
                    {
                        ...
                        string label = (userFriendlyName == string.Empty) ? wrapper3.Label : "wrapper3.Label.FriendlyName";
                        ...
                        {
                            this.updateOrAddKeyframe(wrapper3, state, newValue);
                            if (this.CurvesChanged != null)
                            {
                                CinemaClipCurveWrapper wrapper4 = base.Wrapper as CinemaClipCurveWrapper;
                                if (wrapper4 != null)
    ...
    {
        get { return this.haveCurvesChanged; }
        set
        {
